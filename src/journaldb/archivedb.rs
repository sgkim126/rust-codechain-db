...
impl JournalDB for ArchiveDB {
    ...
    fn journal_under(&mut self, batch: &mut DBTransaction, now: u64, _id: &H256) -> Result<u32, DatabaseError> {
        ...
        for i in self.overlay.drain() {
            let (key, (value, rc)) = i;
            if rc > 0 {
                batch.put(self.column, key.as_ref(), &value);
                inserts += 1;
            }
            if rc < 0 {
                assert_eq!(-1, rc);
    ...
    fn inject(&mut self, batch: &mut DBTransaction) -> Result<u32, DatabaseError> {
        ...
        for i in self.overlay.drain() {
            let (key, (value, rc)) = i;
            if rc > 0 {
                if self.backing.get(self.column, key.as_ref())?.is_some() {
            ...
            if rc < 0 {
                assert_eq!(-1, rc);
                if self.backing.get(self.column, key.as_ref())?.is_none() {
                    return Err(DatabaseError::NegativelyReferencedHash(key))
                }
                batch.delete(self.column, key.as_ref());
                deletes += 1;
            }
        }
